...
export type UserRole = 'GLOBAL_ADMIN' | 'COUNTRY_ADMIN' | 'CFI' | 'INSTRUCTOR' | 'PILOT';
...
/** Returns true if the caller is an instructor, CFI, or admin (not a plain PILOT). */
export function isInstructorOrAbove(role: UserRole): boolean {
  return role === 'INSTRUCTOR' || role === 'CFI' || role === 'COUNTRY_ADMIN' || role === 'GLOBAL_ADMIN';
}

export function isCfiOrAbove(role: UserRole): boolean {
  return role === 'CFI' || role === 'COUNTRY_ADMIN' || role === 'GLOBAL_ADMIN';
}
